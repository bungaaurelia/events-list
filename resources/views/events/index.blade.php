...
<meta name="csrf-token" content="{{ csrf_token() }}">
        ...
        <div class="row container mt-4">
            ...
            <div class="col">
                <div class="text-right mb-3">
                    <form method="GET" action="{{ route('events.index') }}" class="form-inline justify-content-end">
                        ...
                        <!-- Show All Button -->
                        <a href="{{ route('events.index') }}" class="btn btn-secondary mb-2 ml-2">Show All</a>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div id="loader" style="display: none;">
        <p>Loading...</p> <!-- You can replace this with a more sophisticated loader graphic if you like -->
    </div>
        ...
        <tbody>
            @foreach ($publishedEvents as $event)
                <tr>
                    ...
                    <td width="110">{{ $event->event_date }}</td>
                    <td>{{ $event->event_time }}</td>
                    <td>{{ $event->organizer->organizer_name }}</td>
                    <td>{{ $event->eventType->event_type_name }}</td>
                    <td>{{ $event->description }}</td>
                    <td><input type="checkbox" class="status-toggle" data-id="{{ $event->id }}" {{ $event->status == 'published' ? 'checked' : '' }}>
                    </td>
                    <td width="160">
